...
_ADDRESS_ACCEL = const(0x6B)  # (0x32 >> 1)       // 0011001x
...
class LSM303_Accel:  # pylint:disable=too-many-instance-attributes
    ...
    **Quickstart: Importing and using the device**
    ...
    def _raw_acceleration(self):
     #   if self.regAttempt == 0:
      #      print("------------------------------------------------------------------")

        self._read_bytes(
            self._accel_device, _REG_ACCEL_OUT_X_L_A | 0x80, 6, self._BUFFER
            #self._accel_device, self.regToRead, 6, self._BUFFER
        )
#        self.regAttempt += 1
 #       print(f"Raw accel (#{self.regAttempt} from {'{:02X}'.format(self.regToRead)}: {self._BUFFER}")
  #      if self.regAttempt == 5:
   #         self.regToRead += 1
    #        self.regAttempt = 0
